...
remove them from the AWS load balancer target group before rebooting:

1. Login to AWS Console. Switch to the prod environment (`gds aws govuk-production-<your-role> -l`)
...
1. Remove the machine from the following [Target Groups](https://eu-west-1.console.aws.amazon.com/ec2/home?region=eu-west-1#TargetGroups:sort=targetGroupName):
   ...
   1. cache-www
   1. cache-www-origin
1. Check traffic again (should only be Smokey healthchecks now).
1. Run the `vm.reboot` fab script on your local machine like normal.
1. Re-add the machine to the above target groups
1. Check the traffic is flowing again with `tail -f /var/log/nginx/lb-access.log` - there should be some requests from the load balancer.
