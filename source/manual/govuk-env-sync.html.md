...
If you get an Icinga alert about a failing task, check `/var/log/syslog` and `/var/log/syslog.1` on the relevant machine (usually `db_admin`). If the problem appears to be a one-off, consider acking the alert and seeing if it fails again the next day before investing time in debugging.
